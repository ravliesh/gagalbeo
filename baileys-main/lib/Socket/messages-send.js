...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            // Fix Native Flow & Order Message Attributes
            if (messages.interactiveResponseMessage?.nativeFlowResponseMessage) {
                extraAttrs['native_flow_name'] = messages.interactiveResponseMessage?.nativeFlowResponseMessage.name
            }
            if (messages.interactiveMessage?.nativeFlowMessage) {
                const flowName = messages.interactiveMessage.nativeFlowMessage.buttons?.[0]?.name
                if (flowName) extraAttrs['native_flow_name'] = flowName
            }
            if (messages.viewOnceMessage?.message?.interactiveMessage?.nativeFlowMessage) {
                 const flowName = messages.viewOnceMessage.message.interactiveMessage.nativeFlowMessage.buttons?.[0]?.name
                 if (flowName) extraAttrs['native_flow_name'] = flowName
            }
            // Tambahkan dukungan eksplisit untuk orderMessage
            if (messages.orderMessage) {
                extraAttrs['mediatype'] = 'order'
            }
            ...
            if (isGroup || isStatus) {
                ...
                for (const { user, device } of devices) {
                    ...
                    isMe ? meJids.push(jid) : otherJids.push(jid)
                    allJids.push(jid)
                }
            ...
            const stanza = {
                tag: 'message',
                attrs: { id: msgId, type: getTypeMessage(messages), ...(additionalAttributes || {}) },
            ...
            if (shouldIncludeDeviceIdentity) {
                stanza.content.push({ tag: 'device-identity', attrs: {}, content: (0, Utils_1.encodeSignedDeviceIdentity)(authState.creds.account, true) });
            }
    ...
    const getMediaType = (message) => {
        ...
        if (message.orderMessage) return 'order' // Penting untuk Order Message
    ...
    const getButtonType = (message) => {
        ...
        if (message.orderMessage) return 'order' // Deteksi order sebagai tipe button
        if (message.listMessage) return 'list'
        if (message.buttonsMessage) return 'buttons'
        
        const flowName = message.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name
        if (flowName === 'review_and_pay') return 'review_and_pay'
        if (flowName === 'review_order') return 'review_order'
        if (flowName === 'payment_info') return 'payment_info'
        if (flowName === 'payment_status') return 'payment_status'
        if (flowName === 'payment_method') return 'payment_method'
        if (message.interactiveMessage && message.interactiveMessage?.nativeFlowMessage) return 'interactive'
        return null
    }
    ...
    return {
                        ...
                        if (result.error) error = result.error;
                            ...
                            try {
                                ...
                                if (media.result !== WAProto_1.proto.MediaRetryNotification.ResultType.SUCCESS) {
                                    throw new boom_1.Boom(`Media retry failed`, { statusCode: 404 });
                                }
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Auto wrap nativeFlowMessage
            if (content.nativeFlowMessage) {
                content = {
                    viewOnceMessage: {
                        message: {
                            interactiveMessage: {
                                nativeFlowMessage: content.nativeFlowMessage,
                                ...content
                            }
                        }
                    }
            ...
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content && WABinary_1.isJidGroup(jid)) {
                ...
                if (messageType) {
                    switch(messageType) {
                        case 'PAYMENT':
                            ...
                            return await relayMessage(jid, paymentContent, { messageId: Utils_1.generateMessageID(), ...getParticipantAttr() });
                        case 'PRODUCT':
                        ...
                        case 'ALBUM': return await rahmi.handleAlbum(content, jid, quoted)
                        case 'EVENT': return await rahmi.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT': return await rahmi.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY': return await rahmi.handleGroupStory(content, jid, quoted)
                    }
                }
                ...
                const fullMsg = await Utils_1.generateWAMessage(jid, content, {
                    ...
                    upload: async (readStream, opts) => waUploadToServer(readStream, { ...opts, newsletter: WABinary_1.isJidNewsLetter(jid) }),
